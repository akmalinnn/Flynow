...
fun Collection<DepartureFlight>?.toDepartureFlights() = this?.map { it.toFlight() } ?: listOf()

fun ReturnFlight?.toFlight() =
    Flight(
        id = this?.id?: 0,
        flightCode = this?.flightCode.orEmpty(),
        terminal = this?.terminal.orEmpty(),
...
fun Collection<ReturnFlight>?.toReturnFlights() = this?.map { it.toFlight() } ?: listOf()
